...
func TestBeEmpty(t *testing.T) {
	...
	testCases := map[string]testCase{
		...
		"Empty chan matches":     {actual: lang.ChanOf[int](), expectedOutcome: ExpectSuccess},
		"Non-empty chan fails":   {actual: lang.ChanOf[int](1, 2, 3), expectedOutcome: ExpectFailure, expectFailurePattern: `Expected '.+' to be empty, but it is not \(has a length of 3\)`},
